...
def getRotMat(RPY):
    cosR, cosP, cosY = map(cos, RPY)
    sinR, sinP, sinY = map(sin, RPY)
...
def getTransformMat(params_cam, params_lidar):
    ...
    camRPY = camRPY + np.array([-90*pi/180,0,-90*pi/180])
...
def getCameraMat(params_cam):
    ...
    focalLength = params_cam["WIDTH"]/(2*tan(params_cam["FOV"]/2*pi/180))
...
if __name__ == '__main__':    
    rospy.init_node('sensor_camera_lidar_calib', anonymous=True)
