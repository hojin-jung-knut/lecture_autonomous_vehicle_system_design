...
class PurePursuit:
    ...
    def pure_pursuit(self):
        self.is_look_forward_point = False
        rear_axle_pos_x = self.pose.x - self.vehicle_length*cos(self.yaw)*0.5
        rear_axle_pos_y = self.pose.y - self.vehicle_length*sin(self.yaw)*0.5
        temp = np.zeros((1, 2))
        for p in self.path.poses:
            dist = sqrt((p.pose.position.x-rear_axle_pos_x)**2 + (p.pose.position.y-rear_axle_pos_y)**2)
            if dist >= self.lfd:
        ...
        if self.is_look_forward_point:
            alpha = atan2(temp[0][1] - rear_axle_pos_y, temp[0][0] - rear_axle_pos_x) - self.yaw
            self.lfd = self.vel * 0.7
            if self.lfd < 5: self.lfd = 5
            self.ctrl_cmd_msg.steering = atan2(2 * self.vehicle_length * sin(alpha), self.lfd)
